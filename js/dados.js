var MENU = {
    "carne": [
        {
            "id": "ribs-brisket-and-burnt-ends",
            "img": "./img/cardapio/file/filecarne.png",
            "name": "BAURU",
            "dsc": "File, Queijo e tomate na chapa",
            "price": 30
        },
        {
            "id": "005-kings-carolina-oink-sampler",
            "img": "./img/cardapio/file/filecarne.png",
            "name": "X FILE ",
            "dsc": "File e Queijo",
            "price": 28
        },
        {
            "id": "texas-monthlys-1-bbq-brisket",
            "img": "./img/cardapio/file/filecarne.png",
            "name": "X FILE SALADA",
            "dsc": "File, queijo e Salada",
            "price": 30
        },
        {
            "id": "whole-brisket-texas-bbq-sauce",
            "img": "./img/cardapio/file/filecarne.png",
            "name": "X FILE BACON",
            "dsc": "File, queijo e bacon",
            "price": 32
        },
        {
            "id": "whole-texas-smoked-brisket",
            "img": "./img/cardapio/file/filecarne.png",
            "name": "X FILE BACON SALADA",
            "dsc": "File, queijo, bacon e salada",
            "price": 34
        },
        {
            "id": "mini-trinity-bbq-combo-brisket-ribs-and-links",
            "img": "./img/cardapio/file/filecarne.png",
            "name": "X FILE EGG",
            "dsc": "File, queijo e ovo",
            "price": 29
        },
        {
            "id": "235203-blue-smoke-baby-back-ribs-backyard-barbecue-chicken-combo",
            "img": "./img/cardapio/file/filecarne.png",
            "name": "X FILE EGG SAL",
            "dsc": "File, queijo, ovo e salada",
            "price": 32
        },
        {
            "id": "006-kings-meat-lovers-special",
            "img": "./img/cardapio/file/filecarne.png",
            "name": "X FILE EGG BC",
            "dsc": "File, queijo, ovo e bacon",
            "price": 34
        },
        {
            "id": "the-big-ugly-bbq-dinner-for-6",
            "img": "./img/cardapio/file/filecarne.png",
            "name": "X FILE EGG BC SAL",
            "dsc": "The Big Ugly BBQ Dinner for 6",
            "price": 36
        },
        {
            "id": "17796-mighty-quinns-bbq-sampler-pack",
            "img": "./img/cardapio/file/filecarne.png",
            "name": "X FILE TUDO",
            "dsc": "File, bacon, queijo, ovo, calabresa, presunto e salada",
            "price": 39
        },
    ],
    "burgers": [
        {
            "id": "the-gramercy-tavern-burger-4-pack",
            "img": "./img/cardapio/burguers/burguer2.png",
            "name": "HAMBURGUER",
            "dsc": "Pão e Hamburguer",
            "price": 16
        },
        {
            "id": "shake-shack-shackburger-8-pack",
            "img": "./img/cardapio/burguers/burguer2.png",
            "name": "X BURGUER",
            "dsc": "Hamburguer e queijo.",
            "price": 19
        },
        {
            "id": "gotts-cheeseburger-kit-for-4",
            "img": "./img/cardapio/burguers/burguer2.png",
            "name": "X SALADA",
            "dsc": "Hamburguer, queijo e salada.",
            "price": 21
        },
        {
            "id": "le-big-matt-kit-for-6",
            "img": "./img/cardapio/burguers/burguer2.png",
            "name": "X BACON SALADA",
            "dsc": "Hamburguer, queijo, bacon e salada.",
            "price": 28
        },
        {
            "id": "shake-shack-shackburger-16-pack",
            "img": "./img/cardapio/burguers/burguer2.png",
            "name": "X EG",
            "dsc": "Hamburguer, queijo e ovo.",
            "price": 23
        },
        {
            "id": "21-usda-prime-burgers-pack-of-18-8oz-each",
            "img": "./img/cardapio/burguers/burguer2.png",
            "name": "X EG SALADA",
            "dsc": "Hamburguer, queijo, ovo e salada",
            "price": 25
        },
        {
            "id": "double-stack-burger-kit-for-4",
            "img": "./img/cardapio/burguers/burguer2.png",
            "name": "X EG BACON",
            "dsc": "Hamburguer, queijo, ovo e bacon",
            "price": 27
        },
        {
            "id": "goldbelly-burger-bash-pack",
           "img": "./img/cardapio/burguers/burguer2.png",
            "name": "X EG BACON SAL",
            "dsc": "Hamburguer, queijo, bacon e salada",
            "price": 29
        },
        {
            "id": "burger-au-poivre-kit-4-pack",
           "img": "./img/cardapio/burguers/burguer2.png",
            "name": "X TUDO",
            "dsc": "Hamburguer, bacon, queijo, ovo, calabresa, presunto e salada",
            "price": 34
        },
        {
            "id": "burger-au-poivre-kit-4-pack",
            "img": "./img/cardapio/burguers/burguer2.png",
            "name": "X MAIS SABOR",
            "dsc": "Hamburguer, bacon, queijo, ovo, calabresa, presunto, frango, catupiry e salada",
            "price": 40
        },
    ],
    "hotdog": [
        {
            "id": "15259-german-chocolate-killer-brownie-tin-pack",
            "img": "./img/cardapio/dog/salsicha.png",
            "name": "DOG SIMPLES",
            "dsc": "Salsicha, maionese, catchup, mostarda, vinagrete e batata palha",
            "price": 15
        },
        {
            "id": "jacques-world-famous-chocolate-chip-cookies",
            "img": "./img/cardapio/dog/salsicha.png",
            "name": "DOG SALADA",
            "dsc": "Salsicha, maionese, catchup, mostarda, vinagrete, tomate, batata palha e milho",
            "price": 18
        },
        {
            "id": "luigis-original-cannoli-pie",
            "img": "./img/cardapio/dog/salsicha.png",
            "name": "DOG DUPLO",
            "dsc": "2 Salsicha, maionese, catchup, mostarda, vinagrete e batata palha",
            "price": 20
        },
        {
            "id": "sea-salted-caramel-swirl-cheesecake",
            "img": "./img/cardapio/dog/salsicha.png",
            "name": "DOG 2 QUEIJOS",
            "dsc": "Salsicha, maionese, catchup, mostarda, mussarela, catupiry e batata palha",
            "price": 24
        },
        {
            "id": "brooklyn-blackout-cookie-brownie-combo-pack-2-tins",
            "img": "./img/cardapio/dog/salsicha.png",
            "name": "DOG ESPECIAL",
            "dsc": "Salsicha, maionese, catchup, mostarda, mussarela, catupiry, alface, tomate, milho e batata palha",
            "price": 25
        },
    ],
    "presuntoqueijo": [
        {
            "id": "hong-kong-boba-tea-kit-for-6",
            "img": "./img/cardapio/misto/presuntoqueijo.png",
            "name": "MISTO BURGUER",
            "dsc": "Hamb de carne, presunto e queijo",
            "price": 28
        },
        {
            "id": "guys-caliente-margaritas-for-12",
            "img": "./img/cardapio/misto/presuntoqueijo.png",
            "name": "MISTO BURGUER SAL",
            "dsc": "Hamb de carne, presunto, queijo e salada",
            "price": 30
        },
        {
            "id": "woodford-reserve-mint-julep-syrup",
            "img": "./img/cardapio/misto/presuntoqueijo.png",
            "name": "MISTO QUENTE",
            "dsc": "Presunto e queijo",
            "price": 22
        },
        {
            "id": "new-orleans-hurricane-mix",
            "img": "./img/cardapio/misto/presuntoqueijo.png",
            "name": "MISTO QUENTE SAL",
            "dsc": "Presunto, quejo e salada",
            "price": 23
        },
        {
            "id": "margarita-mix",
            "img": "./img/cardapio/misto/presuntoqueijo.png",
            "name": "MISTO BACON",
            "dsc": "Presunto, queijo e bacon",
            "price": 27
        },
        {
            "id": "woodford-reserve-mint-julep-syrup-2-pack",
            "img": "./img/cardapio/misto/presuntoqueijo.png",
            "name": "MISTO BACON SAL",
            "dsc": "Presunto, queijo, bacon e salada",
            "price": 28
        },
        {
            "id": "unicorn-parade-milkshake-kit-for-8",
            "img": "./img/cardapio/misto/presuntoqueijo.png",
            "name": "MISTO EGG",
            "dsc": "Presunto, queijo e ovo",
            "price": 24
        },
        {
            "id": "chickpea-chiller-kit-for-6",
           "img": "./img/cardapio/misto/presuntoqueijo.png",
            "name": "MISTO EGG SAL",
            "dsc": "Presunto, queijo, ovo e salada",
            "price": 25
        },
        {
            "id": "15194-old-honey-barn-mint-julep-mixer-200ml",
            "img": "./img/cardapio/misto/presuntoqueijo.png",
            "name": "MISTO EGG BACON",
            "dsc": "Presunto, queijo, ovo e bacon",
            "price": 28
        },
        {
            "id": "kentucky-derby-mint-julep-gift-set",
            "img": "./img/cardapio/misto/presuntoqueijo.png",
            "name": "MISTO EGG BACON SAL",
            "dsc": "Presunto, queijo, ovo, bacon e salada",
            "price": 30
        },
    ],
    "frango": [
        {
            "id": "2-lou-malnatis-deep-dish-pizzas",
            "img": "./img/cardapio/frango/filefrango.png",
            "name": "X FRANGO",
            "dsc": "Frango e queijo.",
            "price": 23
        },
        {
            "id": "23699-choose-your-own-thin-crust-pizza-4-pack",
            "img": "./img/cardapio/frango/filefrango.png",
            "name": "X FRANGO SALADA",
            "dsc": "Frango, queijo e salada",
            "price": 25
        },
        {
            "id": "choose-your-own-new-haven-style-pizza-6-pack",
            "img": "./img/cardapio/frango/filefrango.png",
            "name": "X FRANGO BACON",
            "dsc": "Frango, queijo, bacon.",
            "price": 28
        },    
        {
            "id": "6-lou-malnatis-deep-dish-pizzas",
            "img": "./img/cardapio/frango/filefrango.png",
            "name": "X FRANGO BC SALADA",
            "dsc": "Frango, queijo, bacon e salada.",
            "price": 30
        },
        {
            "id": "wood-fired-pizzas-best-seller-4-pack",
            "img": "./img/cardapio/frango/filefrango.png",
            "name": "X FRANGO EGG",
            "dsc": "Frango, queijo e ovo.",
            "price": 25
        },
        {
            "id": "236991-choose-your-own-deep-dish-pizza-3-pack",
            "img": "./img/cardapio/frango/filefrango.png",
            "name": "X FRANGO EGG SALADA",
            "dsc": "Frango, queijo, ovo e salada",
            "price": 27
        },
        {
            "id": "choose-your-own-detroit-style-pizza-3-pack",
            "img": "./img/cardapio/frango/filefrango.png",
            "name": "X FRANGO EGG BACON",
            "dsc": "Frango, queijo, ovo e bacon.",
            "price": 30
        },
        {
            "id": "brooklyn-pizza-choose-your-own-5-pack",
            "img": "./img/cardapio/frango/filefrango.png",
            "name": "X FRANGO EGG BACON SALADA",
            "dsc": "Frango, queijo, ovo, bacon e salada.",
            "price": 32
        },
        {
            "id": "choose-your-own-chicago-deep-dish-pizza-4-pack",
            "img": "./img/cardapio/frango/filefrango.png",
            "name": "X FRANGO TUDO",
            "dsc": "Frango, queijo, ovo, bacon, calabresa, presunto e salada.",
            "price": 36
        },
    ],
    "calabresa": [
        {
            "id": "california-reserve-filet-mignon-steaks-gift-box",
            "img": "./img/cardapio/calabresa/lanchecalabresa.png",
            "name": "X CALABRESA",
            "dsc": "Calabresa e Queijo",
            "price": 23
        },
        {
            "id": "steaks-and-cakes-date-night-dinner-for-2",
            "img": "./img/cardapio/calabresa/lanchecalabresa.png",
            "name": "X CALABRESA SAL",
            "dsc": "Calabresa, Queijo e salada",
            "price": 25
        },
        {
            "id": "Prime-holiday-steak-sampler-for-10-12",
            "img": "./img/cardapio/calabresa/lanchecalabresa.png",
            "name": "X CALABRESA BACON",
            "dsc": "Calabresa, Queijo e bacon",
            "price": 28
        },
        {
            "id": "bone-in-rib-steak",
            "img": "./img/cardapio/calabresa/lanchecalabresa.png",
            "name": "X CAL BACON SAL",
            "dsc": "Calabresa, quejo, bacon e salada.",
            "price": 30
        },
        {
            "id": "american-wagyu-gold-grade-top-sirloins",
            "img": "./img/cardapio/calabresa/lanchecalabresa.png",
            "name": "X CAL EGG",
            "dsc": "Calabresa, queijo e ovo",
            "price": 25
        },
        {
            "id": "2-peter-luger-steak-pack-b",
            "img": "./img/cardapio/calabresa/lanchecalabresa.png",
            "name": "X CAL EGG SAL",
            "dsc": "Calabresa, queijo, ovo e salada",
            "price": 27
        },
        {
            "id": "ribeye-prime-steak-gift-box",
            "img": "./img/cardapio/calabresa/lanchecalabresa.png",
            "name": "X CAL EGG BACON",
            "dsc": "Calabresa, queijo, ovo e bacon",
            "price": 30
        },
        {
            "id": "dry-aged-usda-prime-black-angus-porterhouse-steak-2-pack",
            "img": "./img/cardapio/calabresa/lanchecalabresa.png",
            "name": "X CAL EGG BACON SAL",
            "dsc": "Calabresa, queijo, ovo, bacon e salada",
            "price": 32
        },
        {
            "id": "california-reserve-ribeye-steak",
            "img": "./img/cardapio/calabresa/lanchecalabresa.png",
            "name": "X CALABRESA TUDO",
            "dsc": "Calabresa, bacon, quejo, ovo, presunto e queijo.",
            "price": 35
        },
    ]
}